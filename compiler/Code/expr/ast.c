...
uint64_t *make_terms(uint64_t *term, uint64_t *other) {
    if (is_empty(other)) {
        ...
        return triple(fst(get_elt(other)), term,
                      make_terms(snd(get_elt(other)), next_elt(other)));
    }
}
...
uint64_t *make_factors(uint64_t *factor, uint64_t *other) {
    if (is_empty(other)) {
        ...
        return triple(fst(get_elt(other)), factor,
                      make_factors(snd(get_elt(other)), next_elt(other)));
    }
}
